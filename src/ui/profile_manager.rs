                   ...
                   match profile.update(message) {
                       profile::ProfileAction::Edit(idx,data)  => {
                           if let Some(index) = idx {
                               self.window = Window::ProfileEdit(profile_edit::ProfileEdit::new(idx, data[index].clone()));
                           } else {
                               self.window = Window::ProfileEdit(profile_edit::ProfileEdit::new(None, ActionRecord::new("", "", vec!())));
                           }
                       }
                       profile::ProfileAction::None => (),
                       profile::ProfileAction::Close(data) => return ProfileWindowAction::Close(data),
                   }
               }
           }
               ...
               if let Window::ProfileEdit(edit) = &mut self.window {
                   match edit.update(message) {
                       profile_edit::ProfileEditAction::Save(idx,data) => {
                           ...
                           self.window = Window::Profile(profile::Profile::new(self.stable_profile.clone()));
                       }
